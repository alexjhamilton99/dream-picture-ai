...
func Required() RuleSet {
	return RuleSet{
		...
		ValidateFunc: func(ruleSet RuleSet) bool {
			str, ok := ruleSet.FieldValue.(string)
			if !ok {
				return false
			}
			return len(str) > 0
...
func Equals(s string) RuleFunc {
	return func() RuleSet {
		return RuleSet{
			...
			MessageFunc: func(set RuleSet) string {
				return fmt.Sprintf("%s should equal %s", set.FieldName, s)
			},
		}
	}
}
...
func Max(n int) RuleFunc {
	return func() RuleSet {
		return RuleSet{
			...
			MessageFunc: func(set RuleSet) string {
				return fmt.Sprintf("%s should be maximum of %d characters long", set.FieldName, n)
			},
		}
	}
}
